...
def filt_saltos(i, res, nota):
    # saltos consecutivos em mesma direção
    if i >= 3:
...
def filt_pll(i, cf, res, nota):
    ...
    # paralelismo de CIs por mais de 4 vezes
    if i > 2:
        if int_har_mus(cf[i], nota) == int_har_mus(cf[i - 1], res[i - 1])\
                and int_har_mus(cf[i - 1], res[i - 1]) == int_har_mus(cf[i - 2], res[i - 2])\
                and int_har_mus(cf[i - 2], res[i - 2]) == int_har_mus(cf[i - 3], res[i - 3])\
                and int_har_mus(cf[i - 3], res[i - 3]) == int_har_mus(cf[i - 4], res[i - 4]):
            return True

    # CPs intermitentes
    if i > 3:
        i_1 = int_har(cf[i], nota)
        i_2 = int_har(cf[i - 2], res[i - 2])
        i_3 = int_har(cf[i - 4], res[i - 4])
        if i_1 in conper\
                and i_1 == i_2\
                and i_2 == i_3:
            return True
...
# filtros parciais
def filtp_grau_conj(i, res, nota, fila):
    ...
    elif len(fila) > f_1 and int_har(nota, res[i - 1]) <= i_2 and int_har(nota, res[i - 1]) != 0:
        return False
    elif len(fila) > f_2:
        return False
    else:
...
def filtp_saltos(i, res, nota, fila):
    f = 15

    # saltos dissonantes compostos (Schoenberg, p. 23)
    if len(fila) > f:
        return False

    elif i >= 2:
        # ascendentes
        if int_mel(res[i - 2], res[i - 1]) > 0 \
                and int_mel(res[i - 1], nota) > 0 \
                and int_mel(res[i - 2], nota) > 9:
            return True
...
def filtp_escada(i, res, nota):
    # filtra 4 ou mais movimentos na mesma direção
    if i > 3:
        if int_mel(nota, res[i - 1]) > 0\
                and int_mel(res[i - 1], res[i - 2]) > 0\
                and int_mel(res[i - 2], res[i - 3]) > 0\
                and int_mel(res[i - 3], res[i - 4]) > 0:
            return True
        if int_mel(nota, res[i - 1]) < 0 \
                and int_mel(res[i - 1], res[i - 2]) < 0 \
                and int_mel(res[i - 2], res[i - 3]) < 0 \
