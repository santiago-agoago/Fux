...
def gerar_nota(contra):
    ...
    if nota2 not in naturais:
        ...
        return nota2
#intervalo
def int_abs(nota1, nota2):
    return abs(nota1 - nota2)
def int_rel(nota, notal):
    return notal - nota
...
#        return True
#
#    #filtra consocâncias perfeitas por movimento direto
#    if nota - res[i - 1] > 0 and cf[i] - cf[i - 1] > 0:
#        if abs(cf[i] - nota) in conper:
#            return True
#    if nota - res[i - 1] < 0 and cf[i] - cf[i - 1] < 0:
#        if abs(cf[i] - nota) in conper:
#            return True
def filt_pll(i, cf, res, nota):
    if int_abs(cf[i], nota) in conper:
        if int_abs(cf[i - 1], res[i - 1]) in conper:
            return True
        if int_rel(res[i - 1], nota) > 0 and int_rel(cf[i - 1], cf[i]) > 0:
            return True
        if int_rel(res[i - 1], nota) < 0 and int_rel(cf[i - 1], cf[i]) < 0:
            return True
    ...
    while completou == False:
        ...
        for i in range(1, len(cf.modo)):
            ...
            if len(res) == len(cf.modo) - 1:
                if not filt_pll(i, cf.modo, res, res[0]):
                    res.append(res[0])
                    completou = True
                    print("\n")
                ...
                or filt_pll(i, cf.modo, res, nota2)\
                or filt_rep(i, res, nota2):
                    it += 1
                    break
